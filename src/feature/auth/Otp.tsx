...
export default function OtpVerification() {
  ...
  const handleInputChange = (
    ...
    // Prevent multiple digits in one input
  ...
  // Updated paste handler - works on all inputs
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    if (/^\d{6}$/.test(pastedData)) {
      ...
      pastedData.split("").forEach((digit, idx) => {
        ...
        if (targetIndex < 6) {
          setValue(`otp.${targetIndex}`, digit);
          if (inputRefs.current[targetIndex]) {
            inputRefs.current[targetIndex]!.value = digit;
          }
        }
      });
  ...
  return (
    <div className="max-w-[540px] lg:w-[540px] h-auto mx-auto bg-[#FFF] p-6 rounded-2xl border border-[#6E51E01A] shadow-[0_0_20px_0_rgba(110,81,224,0.10)]">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="flex justify-between">
          {[0, 1, 2, 3, 4, 5].map((index) => (
            <input
              ...
              inputMode="numeric"
              maxLength={1}
              className={`h-12 w-12 rounded-md border border-gray-300 text-center text-xl focus:border-primaryColor focus:outline-none focus:ring-0.5 focus:primaryColor ${
                activeInput === index
              ...
              onPaste={handlePaste} // Apply to ALL inputs
              autoComplete="off"
            />
          ))}
        </div>
        ...
        <PrimaryButton
          type="submit"
          loading={isLoading || verifyLoadinng}
          text="Submit"
          className="w-full"
        />
      </form>
