...
// ✅ Fix 1: Correct CriticalHeatExchanger options (was using FILL_TYPES before)
...
export default function EditCoolingWaterAsset() {
  ...
  useEffect(() => {
    ...
    // ✅ Fix 2a: Set selectedCustomerId so the reactive effect below can fill site name
    if (asset.customerId) {
      setSelectedCustomerId(asset.customerId);
    }

    // ✅ Sync local supply/return temp state for Delta T calculation
    if (asset.supplyTemperature != null) {
      setSupplyTempVal(String(asset.supplyTemperature));
    }
    if (asset.returnTemperature != null) {
    ...
    reset({
      ...
      systemVolumeUnit:
        (asset.systemVolumeType as "gallons" | "liters") ?? "gallons",
      // ✅ Fix 5: Load evaporation factor from asset instead of hardcoding
      evaporationFactor:
        asset.evaporationFactor != null
          ? String(asset.evaporationFactor)
          : "0.85",
      supplyTemperature:
        asset.supplyTemperature != null
          ? String(asset.supplyTemperature)
  ...
  // ✅ Fix 2b: Reactively fill customerName + siteNameLocation whenever
  //    selectedCustomerId or customers list becomes available
  useEffect(() => {
    if (!selectedCustomerId || customers.length === 0) return;
    const matched = customers.find((c) => c.id === selectedCustomerId);
    if (matched) {
      setValue("customerName", matched.name);
      setValue("siteNameLocation", `${matched.siteName} – ${matched.location}`);
    }
  }, [selectedCustomerId, customers]);

  // ✅ Fix 3: Tonnage auto-calc (was missing from Edit entirely)
          ...
          {isCoolingTowerOrEvap && (
            <>
              {isCoolingTower && (
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                    ...
                    <div>
                      <Label>Critical Heat Exchanger Design</Label>
                      {/* ✅ Fix 1: Use CriticalHeatExchanger options, not FILL_TYPES */}
                      <Controller
                        name="criticalHeatExchangerDesign"
                        control={control}
                        render={({ field }) => (
              ...
              <div className="border border-gray-200 rounded-xl mb-6 overflow-hidden">
                <div className="flex flex-col-reverse lg:flex-row-reverse">
                  ...
                  <div className="flex-1 p-5">
                    <Label required>Recirculation Rate</Label>
                    <div className="flex gap-2 mt-1">
                      {/* ✅ Fix 3: Lock recirculation input when tonnage drives it */}
                      <Input
                        type="number"
                        className="flex-1 min-w-0"
                        placeholder="e.g. 1000"
                        readOnly={!!watchedTonnage}
                        style={
                          watchedTonnage
                  ...
                  <div className="flex-1 p-5">
                    <Label>Tonnage of Cooling</Label>
                    <div className="flex gap-2 mt-1">
                      {/* ✅ Fix 3: Clear recirculation when tonnage is cleared */}
                      <Input
                        type="number"
                        className="flex-1 min-w-0"
                        placeholder="e.g. 350"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                ...
                <div>
                  {/* ✅ Fix 5: evaporationFactor is now loaded from asset data */}
                  <Label>
                    Evaporation Factor{" "}
                    <span
                      className="inline-flex items-center justify-center w-4 h-4 rounded-full border border-gray-300 text-gray-400 text-xs ml-1 cursor-help"
